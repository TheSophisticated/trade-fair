...
with st.container():
    col1, second_col = st.columns([2,1.369])
...
with st.container():
    ...
    with col1:
        ...
        with st.container():
            ...
            with col1:
                ui.metric_card(title="Shares Owned : ", content=st.session_state.companies[company]['shares_bought'], description="Total Shares of Company", key="card1")
            with col2:
                ui.metric_card(title="Shares Price : ", content=st.session_state.companies[company]['share_price'], description="Current Share Price of Company", key="card2")
